...
# Your name: Xinyi Peng
# Your student id: 82080230
# Your email: xinyip@umcih.edu
...
# I used Gemni to check my functions and with one debugging that I didn't figured out by myself.
# Did your use of GenAI on this assignment align with your goals and guidelines in your Gen AI contract? If not, why?
# Yes, I used it only when I tried by myself first.
...
def get_longest_lifespan_breed(cache_file):
    ...
    for url, payload in cache.items():
        try:
            ...
            life_max = payload["data"]["attributes"]["life"]["max"]
            if not isinstance(life_max, int):
                continue
        except (KeyError, TypeError):
            continue

        if life_max > max_lifespan:
            max_lifespan = life_max
            best_breeds = [name]
        elif life_max == max_lifespan:
            best_breeds.append(name)

    if not best_breeds:
        return "No breeds found"

    best_breeds.sort()
    return (best_breeds[0], max_lifespan)
    pass
...
def get_groups_above_cutoff(cutoff, cache_file):
    ...
    return {k: v for k, v in group_counts.items() if v >= cutoff}
    pass
